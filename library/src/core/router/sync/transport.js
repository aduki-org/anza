 ...
 * Factory receives { url, params } for named param access.
 * Returns a disposer that removes the connection factory and closes active connection.
 */
export function registerConnection(patternStr, factoryFn) {
  registry.set(patternStr, { factoryFn, pattern: null });

  return () => {
    registry.delete(patternStr);
    const conn = activeConnections.get(patternStr);
    if (conn) {
      try {
...
export async function coordinateConnections(url) {
  ...
  for (const patternStr of matchedPatterns) {
    if (!activeConnections.has(patternStr)) {
      ...
      // Extract named params for the factory
      const result = entry.pattern.exec(targetUrl.href);
      const params = result?.pathname?.groups ?? {};
      try {
        const conn = await entry.factoryFn({ url: targetUrl, params });
        if (conn) {
