...
describe('Declarative Route-Coupling and Fluent Transitions', () => {
  ...
  it('should cast path parameters, query variables, and hash based on element props definitions', async () => {
    ui.element('test-casting-page', {
      url: '/cast/:active/:count/:name',
      container: '#test-container',
      query: ['q', 'page', 'flag'],
      props: {
        active: { type: Boolean },
      ...
      template: '<div>Cast page</div>'
    });

    router.registerContainer('#test-container', container);

    await new Promise((resolve, reject) => {
      router.nav.to('/cast/true/42/alice?q=hello&page=3&flag=1#section-1')
        .on('found', () => resolve())
        .on('error', (err) => reject(err));
    });

    const mountedEl = container.querySelector('test-casting-page');
    if (!mountedEl) {
      throw new Error('Expected element to be mounted inside container');
    }

    if (mountedEl.active !== true) {
      throw new Error(`Expected active to be cast to true, got ${mountedEl.active}`);
    }
    if (mountedEl.count !== 42) {
      throw new Error(`Expected count to be cast to 42, got ${mountedEl.count}`);
    }
    if (mountedEl.name !== 'alice') {
      throw new Error(`Expected name to be "alice", got ${mountedEl.name}`);
    }
    if (mountedEl.q !== 'hello') {
      throw new Error(`Expected q to be "hello", got ${mountedEl.q}`);
    }
    if (mountedEl.page !== 3) {
      throw new Error(`Expected page to be cast to 3, got ${mountedEl.page}`);
    }
    if (mountedEl.flag !== true) {
      throw new Error(`Expected flag to be cast to true, got ${mountedEl.flag}`);
    }
    if (mountedEl.hash !== '#section-1') {
      throw new Error(`Expected hash to be "#section-1", got ${mountedEl.hash}`);
    }
  });
